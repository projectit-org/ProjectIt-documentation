...
/**
 *   Class SomeOtherModelUnitSyntaxAnalyser is ... TODO
 *
 */
export class SomeOtherModelUnitSyntaxAnalyser implements SyntaxAnalyser {
    locationMap: any;
    ...
    private transformSharedPackedParseTreeNode(node: SPPTNode): any {
        try {
            ...
            if (e.message.startsWith("Syntax error in ") || e.message.startsWith("Error in SomeOtherModelUnitSyntaxAnalyser")) {
                throw e;
            } else {
    ...
    private transformSharedPackedParseTreeBranch(branch: SPPTBranch): any {
        ...
        if ("SomeOtherModelUnit" == brName) {
            return this.transformSomeOtherModelUnit(branch);
            ...
            throw new Error(`Error in SomeOtherModelUnitSyntaxAnalyser: ${brName} not handled for node '${branch?.matchedText}'`);
        }
    }
     ...
     * @private
     */
    private transformSomeOtherModelUnit(branch: SPPTBranch): SomeOtherModelUnit {
        ...
        somePart = this.transformSharedPackedParseTreeNode(children[3]); // RHSPartEntry

        return SomeOtherModelUnit.create({ name: name, somePart: somePart });
    }
    /**
     * Method to transform branches that match the following rule:
